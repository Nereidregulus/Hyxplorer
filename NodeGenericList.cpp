...
NodeGenericList::NodeGenericList(std::span<const char> buffer)
{
	...
	while (!end_of_node)
	{
		...
		add(std::make_shared<TreeNode>(std::span(buffer).subspan(child_pos)));
...
NodeGenericList::~NodeGenericList()
{
